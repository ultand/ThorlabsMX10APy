...
class MX10A:
    ...
    def _instrument_query(self, query: str) -> str:
        try:
            ...
            e_string = f"Hardware Communication Error during {query}: {e}"
    ...
    def _instrument_write(self, command: str):
        try:
            ...
            self.logger.info("Sending command: *OPC?")
            self.inst.query("*OPC?")
            self.check_errors()
            return True

        except VisaIOError as e:
            e_string = f"Hardware Communication Error during command {command}: {e}"
            self.logger.error(e_string)
            raise (e)
    ...
    def amplifier_gain_mode(self) -> str:
        ...
        return "Analog" if mode == "0" else "Digital"
    ...
    def amplifier_gain_mode(self, value: str):
        if value.lower() == "digital":
            ...
            e_string = (
                "Error: provided amplifier gain mode is neither 'digital' nor 'analog'. "
                "Value has not been updated. Please check provided value."
            )
    ...
    def amplifier_gain(self, value: float):
        if (value < AMPLIFIER_GAIN_MINIMUM) or (value > AMPLIFIER_GAIN_MAXIMUM):

            e_string = (
                f"Error: Provided amplifier gain {value} is outside of software defined "
                f"range {AMPLIFIER_GAIN_MINIMUM} -- {AMPLIFIER_GAIN_MAXIMUM}. "
                "Value not updated.",
            )
    ...
    def mzm_dither_amplitude(self, value: float):
        ...
        if (value < MZM_DITHER_AMPLITUDE_MINIMUM) or (value > MZM_DITHER_AMPLITUDE_MAXIMUM):
            e_string = (
                f"Error: Provided dither amplitude {value} is outside of software defined "
                f"range {MZM_DITHER_AMPLITUDE_MINIMUM} -- {MZM_DITHER_AMPLITUDE_MAXIMUM}. "
                "Value not updated."
            )
            self.logger.error(e_string)
            raise ValueError(e_string)
    ...
    def mzm_dither_frequency(self, value: float):
        if not (MZM_DITHER_FREQUENCY_MINIMUM <= value <= MZM_DITHER_FREQUENCY_MAXIMUM):
            e_string = (
                f"Error: Provided dither frequency {value} is outside of software defined "
                f"range {MZM_DITHER_FREQUENCY_MINIMUM} -- {MZM_DITHER_FREQUENCY_MAXIMUM}. "
                "Value not updated."
            )
            raise ValueError(e_string)
    ...
    def mzm_hold_ratio(self) -> float:
        ...
        if "auto power" not in current_state.lower():
            ...
            self.logger.warning(
                f"ERROR: Current MZM state is {current_state}. MZM hold ratio only has an "
                "effect in auto power modes. Update MZM mode accordingly."
            )
    ...
    def mzm_hold_ratio(self, value: float):
        ...
        if not (MZM_HOLD_RATIO_MINIMUM <= value <= MZM_HOLD_RATIO_MAXIMUM):
            e_string = (
                f"ERROR: Provided hold ratio {value} is outside of software defined range "
                f"{MZM_HOLD_RATIO_MINIMUM} - {MZM_HOLD_RATIO_MAXIMUM}. Value not updated."
            )
            self.logger.error(e_string)
            raise ValueError(e_string)
    ...
    def mzm_hold_voltage(self) -> float:
        ...
        current_state = self.mzm_bias_mode
        if current_state.lower() != "manual voltage":
            self.logger.warning(
                "WARNING: MZM mode is not Manual Voltage. Hold voltage is not currently being used."
            )
    ...
    def mzm_hold_voltage(self, value: float):
        if (value < MZM_MANUAL_VOLTAGE_MINIMUM) or (value > MZM_MANUAL_VOLTAGE_MAXIMUM):

            e_string = (
                f"ERROR: Provided manual voltage {value} is outside software defined range "
                f"{MZM_MANUAL_VOLTAGE_MINIMUM} - {MZM_MANUAL_VOLTAGE_MAXIMUM}. "
    ...
    def mzm_bias_mode(self) -> str:
        ...
        match mode:
            ...
            case _:
                raise ValueError(f"Error: MZM Bias mode query has returned unknown value {mode}.")
    ...
    def mzm_bias_mode(self, value: str):
        match value.lower():
            ...
            case "9" | "auto power ratio neg":
                command = "9"
            case _:
                raise ValueError(f"ERROR: MZM BIAS MODE has returned an unknown value {value}.")
    ...
    def voa_set_attenuation(self, value: float):
        ...
        if not (MIN_VOA_ATTENUATION <= value <= MAX_VOA_ATTENUATION):

            e_string = (
                f"ERROR: Provided VOA attenuation {value} is outside of software defined range"
                f" {MIN_VOA_ATTENUATION} - {MAX_VOA_ATTENUATION}. Value not updated."
            )
    ...
    def check_errors(self):
        try:
            for _ in range(ERROR_QUEUE_LIMIT):
                ...
                if (
                    (not error_string)
                    or (error_string.startswith("0"))
                    or (error_string == "No error")
                ):
                    break

                self.logger.warning(f"MX10A Hardware Error: {error_string}")
    ...
    def close(self):
        if hasattr(self, "inst"):
            try:
                ...
                self.logger.error(
                    "Could not cleanly reset settings during close. Connection may be dead."
                )
            finally:
                try:
                    self.inst.close()
